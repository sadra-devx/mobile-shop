...
export function ProductCard({ product }) {
  ...
  const { id, name, brand, price, discountPercent, images, rating, inStock, createdAt } = product;
      ...
      <div className="relative aspect-square overflow-hidden bg-zinc-100 dark:bg-zinc-900">
        ...
        {imgError || !thumbnail ? (
          ...
          <img
            ...
            className={`h-full w-full bg-white object-cover p-3 transition-all duration-700 ease-out ${
              imgLoaded ? "scale-100 opacity-100 blur-0" : "scale-95 opacity-0 blur-md"
            }`}
          />
        )}
        ...
        <div className="mt-3 flex-1">
          ...
          <div className="flex items-center justify-between gap-2">
            <p className="text-base font-bold text-zinc-900 dark:text-white">
              ...
              <span className="mr-1 text-xs font-normal text-zinc-500">تومان</span>
            </p>
            {numericDiscount > 0 && (
              <span className="rounded-md bg-rose-50 px-1.5 py-0.5 text-[13px] font-bold text-rose-600 dark:bg-rose-900/30 dark:text-rose-400">
